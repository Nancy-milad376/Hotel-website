...
const BookingForm = ({ selectedRoom = null, onBookingComplete = () => {} }) => {
  const [checkInDate, setCheckInDate] = useState(new Date());
  const [checkOutDate, setCheckOutDate] = useState(new Date(new Date().setDate(new Date().getDate() + 1)));
  const [numberOfRooms, setNumberOfRooms] = useState(1);
  const [numberOfGuests, setNumberOfGuests] = useState(2);
  const [roomType, setRoomType] = useState(selectedRoom ? selectedRoom.type : '');
  ...
  const checkAvailability = async () => {
    try {
      ...
      const availableTypes = Object.keys(response.data).filter(
        type => response.data[type].available && response.data[type].availableRooms >= numberOfRooms
      );
      
      setAvailableRoomTypes(availableTypes);
      
      // If the selected room is not available, reset the selection
      if (roomType && (!response.data[roomType] || !response.data[roomType].available || response.data[roomType].availableRooms < numberOfRooms)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!roomType || !availability[roomType] || !availability[roomType].available || availability[roomType].availableRooms < numberOfRooms) {
        setError(`Sorry, the selected room type is no longer available for your dates. Please select a different room or dates.`);
        setLoading(false);
        return;
      }
      ...
      <form onSubmit={handleSubmit} className="booking-form">
            ...
            {roomType && (
              <div className="room-details">
                ...
                <p className="availability">
                  {availabilityData[roomType]?.availableRooms || 0} rooms available for your dates
                </p>
              </div>
            )}
        ...
        {availabilityData && availableRoomTypes.length === 0 && (
          <div className="no-availability">
            <p>Sorry, there are no rooms available for your selected dates. Please try different dates.</p>
          </div>
        )}
      </form>
    </div>
